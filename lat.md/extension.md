...
The default export `reviewLoop(pi: ExtensionAPI)` in `src/index.ts` is the pi extension entry point declared in `package.json` under `pi.extensions`. It registers the `/diffweb` command and wires the session shutdown hook.

## Command registration

`pi.registerCommand("diffweb", ...)` registers the slash command. The handler:
...
Running `/diffweb` while the window is already open brings the existing window forward and shows an info notification; it does not create a second window.
...
After nulling, the next `/diffweb` invocation creates a fresh controller.
